  ...
  try {
    ...
    if (spec.type === 'list') {
      ...
      if (!rows.length) {
        ...
        panel.appendChild(el('ul', { class: 'bar-list' }, rows.slice(0, 8).map((r) => el('li', {}, [
  ...
  if (spec.chart_config === 'daily_sales_vs_expenses') {
    new Chart(canvas, {
      ...
      data: {
        ...
        datasets: [
          { label: t('total_sales'), data: data.map((r) => Number(r.sales_total || 0)),
            borderColor: palette[0], backgroundColor: palette[0] + '20', fill: true, tension: 0.3 },
          { label: t('total_expenses'), data: data.map((r) => Number(r.expenses_total || 0)),
            borderColor: palette[3], backgroundColor: palette[3] + '20', fill: true, tension: 0.3 },
        ],
      },
      options: chartOpts(),
    });
